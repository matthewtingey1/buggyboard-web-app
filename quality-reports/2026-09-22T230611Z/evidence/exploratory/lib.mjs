...
// Let the server process the request, then hold the response for `ms` before the page sees it.
export async function delay(page, glob, method, ms, override) {
...
export async function openRow(page, text) { await row(page, text).getByRole("button").first().click(); }
