...
- Name: diff-regressions.spec.ts >> 06 Escape after choosing a severity closes the create modal without saving
- Location: specs/diff-regressions.spec.ts:10:1
...
Locator:  getByRole('dialog', { name: 'Create bug' })
Expected: hidden
Received: visible
Timeout:  5000ms

Call log:
  - Expect "toBeHidden" getByRole('dialog', { name: 'Create bug' }) with timeout 5000ms
  - waiting for getByRole('dialog', { name: 'Create bug' })
    14 × locator resolved to <div role="dialog" aria-modal="true" class="bug-modal-overlay" aria-labelledby="create-bug-modal-title">…</div>
...
```yaml
- dialog "Create bug":
  - heading "Create bug" [level=2]
  - button "Close"
  - text: Title
  - textbox "Title": "[functional] esc-sev mudakax5872g"
  - text: Severity
  - combobox "Severity"
  - text: Owner
  - textbox "Owner": buggy
  - text: Description
  - textbox "Description": d
  - button "Cancel"
  - button "Save"
```
