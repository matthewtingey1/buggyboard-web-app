...
- Name: diff-regressions.spec.ts >> 09/13 Escape after choosing a state closes the edit modal and discards
- Location: specs/diff-regressions.spec.ts:35:1
...
Locator:  getByRole('dialog', { name: /^Edit bug #\d+$/ })
Expected: hidden
Received: visible
Timeout:  5000ms

Call log:
  - Expect "toBeHidden" getByRole('dialog', { name: /^Edit bug #\d+$/ }) with timeout 5000ms
  - waiting for getByRole('dialog', { name: /^Edit bug #\d+$/ })
    14 × locator resolved to <div role="dialog" aria-modal="true" class="bug-modal-overlay" aria-labelledby="edit-bug-modal-title">…</div>
       - unexpected value "visible"

```

```yaml
- 'dialog "Edit bug #7056"':
  - 'heading "Edit bug #7056" [level=2]'
  - button "Close"
  - text: ID
  - textbox "ID": "7056"
  - text: Title
  - textbox "Title": "[functional] esc-state-edit mudaklp8j5tt"
  - text: Severity
  - combobox "Severity"
  - text: State
  - combobox "State"
  ...
  - textbox "Owner": buggy
  - text: Description
  - textbox "Description": functional gap fixture
  - button "Delete"
  - button "Cancel"
  - button "Save"
```
