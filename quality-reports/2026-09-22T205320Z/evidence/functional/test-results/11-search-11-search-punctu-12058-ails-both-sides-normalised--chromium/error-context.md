...
- Name: 11-search.spec.ts >> 11 search punctuation probe >> 11-S3c query "log-in" matches title "Login fails" (both sides normalised)
- Location: specs/11-search.spec.ts:70:3
...
Locator:  getByRole('table', { name: 'Bugs' }).locator('tbody tr[role="button"]').filter({ has: locator('td:nth-child(3)').filter({ hasText: /^\[functional\] Login fails mud5tn7uiqkz$/ }) })
Expected: 1
Received: 0
Timeout:  5000ms

Call log:
  - Expect "toHaveCount" getByRole('table', { name: 'Bugs' }).locator('tbody tr[role="button"]').filter({ has: locator('td:nth-child(3)').filter({ hasText: /^\[functional\] Login fails mud5tn7uiqkz$/ }) }) with timeout 5000ms
  - waiting for getByRole('table', { name: 'Bugs' }).locator('tbody tr[role="button"]').filter({ has: locator('td:nth-child(3)').filter({ hasText: /^\[functional\] Login fails mud5tn7uiqkz$/ }) })
...
- generic [ref=e3]:
  - banner [ref=e4]:
    ...
    - generic [ref=e9]:
      - generic [ref=e10]:
        - search "Search bugs by title" [active] [ref=e11]: log-in
